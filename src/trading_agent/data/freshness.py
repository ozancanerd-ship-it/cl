...
_TOLERANCE_DAYS: dict[str, int] = {
    "binance_vision": 45,  # Monatsdateien erscheinen mit bis zu ~5 Wochen Versatz
...
def _tolerance_for(instrument: str) -> int:
    if instrument.endswith("-YF"):
        return _TOLERANCE_DAYS["yahoo"]
    if instrument in ("XAUUSD", "EURUSD", "GBPUSD", "USDJPY"):
        return _TOLERANCE_DAYS["dukascopy"]
    if instrument.endswith("USDT"):
        return _TOLERANCE_DAYS["binance_vision"]
    return _TOLERANCE_DAYS["default"]
